...
const EXPECTED_FIELDS = [
  { key: 'full_name',     label: 'Full Name',     required: false, description: "Athlete's full name" },
  ...
  { key: 'status',        label: 'Status',        required: false, description: 'Active, Injured, or Suspended' },
  { key: 'position',      label: 'Position',      required: false, description: 'Playing position or role' },
  { key: 'jersey_number', label: 'Jersey #',      required: false, description: 'Squad or kit number' },
  { key: 'date_of_birth', label: 'Date of Birth', required: false, description: 'Birth date' },
  ...
  { key: 'nationality',   label: 'Nationality',   required: false, description: 'Primary nationality' },
  { key: 'height',        label: 'Height (cm)',   required: false, description: 'Player height' },
  { key: 'weight',        label: 'Weight (kg)',   required: false, description: 'Player weight' },
]
...
describe('matchHeadersLocally', () => {
  ...
  test('leaves unmatched headers and fields in unmapped arrays', () => {
    const headers = ['Full Name', 'Blood Group', 'Favorite Food']
    const res = matchHeadersLocally(headers)
    ...
    expect(res.unmappedHeaders).toEqual(['Blood Group', 'Favorite Food'])
  })
})

describe('transformAndValidateRows', () => {
  const headers = ['Athlete', 'Birthdate', 'Role', 'Kit No', 'Contact', 'Status']
  const mapping = {
    full_name: 'Athlete',
  ...
  test('successfully maps and validates valid rows with status', () => {
    const rawRows = [
      ['Kwadwo Asamoah', '1998-12-09', 'Midfielder', '10', '0244123456', 'Active'],
      ['Thomas Partey', '1993-06-13', 'CDM', '5', '0200987654', 'Suspended'],
    ]
    ...
    expect(res.rows[0].position).toBe('CM')
    ...
    expect(res.rows[1].status).toBe('Suspended')
    expect(res.rows[1]._valid).toBe(true)
  })

  test('auto-combines First Name and Last Name when Full Name is not provided', () => {
    const customHeaders = ['First Name', 'Last Name', 'Pos', 'No']
    const customMapping = {
      full_name: null,
    ...
    const res = transformAndValidateRows(rawRows, customHeaders, customMapping)
    expect(res.error).toBeNull()
    expect(res.rows[0].full_name).toBe('Ebo Gyamfi')
    expect(res.rows[0].first_name).toBe('Ebo')
    expect(res.rows[0].last_name).toBe('Gyamfi')
    expect(res.rows[0].position).toBe('CDM')
    expect(res.rows[0].back_number).toBe('25')
    expect(res.rows[0].status).toBe('Active')
  })

  test('flags invalid rows and duplicate jersey numbers within file', () => {
    const rawRows = [
      ['Player One', '2000-01-01', 'Striker', '9', '', 'Active'],
      ['Player Two', 'invalid-date', 'UnknownPos', '9', '', 'Injured'],
      ['', '2001-02-02', 'GK', '1', '', 'Active'],
    ]
    ...
    expect(res.rows[1].status).toBe('Injured')
  ...
  test('returns error if neither full_name nor first_name is mapped', () => {
    const unmappedName = { ...mapping, full_name: null, first_name: null, last_name: null }
    const res = transformAndValidateRows([['Row Data']], headers, unmappedName)
    expect(res.error).toBe('Full Name or First Name column must be mapped.')
  })
})

describe('normalisePosition, normaliseStatus & normaliseDate helpers', () => {
  ...
  test('normalises status values', () => {
    expect(normaliseStatus('Active')).toBe('Active')
    expect(normaliseStatus('injured')).toBe('Injured')
    expect(normaliseStatus('Injury')).toBe('Injured')
    expect(normaliseStatus('Suspended')).toBe('Suspended')
    expect(normaliseStatus('red card')).toBe('Suspended')
    expect(normaliseStatus('banned')).toBe('Suspended')
