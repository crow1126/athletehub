    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0A0F1E', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", color: '#E2E8F0' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .pay-nav-link { display: flex; align-items: center; gap: 12px; padding: 11px 14px; border-radius: 12px; text-decoration: none; font-size: 14px; font-weight: 500; transition: all 0.18s; color: #64748B; }
        ...
        <nav style={{ flex: 1, padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {NAV.map(n => (
            <Link
              key={n.href}
              href={n.href}
              className={`pay-nav-link${path === n.href ? ' active' : ''}`}
            >
