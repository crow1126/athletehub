  ...
  return (
    <Layout>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 40px' }}>
        ...
        {/* Filters */}
        <div className="fade-up" style={{ display: 'flex', gap: 10, marginBottom: 22, flexWrap: 'wrap' }}>
        ...
        <div className="card fade-up fade-up-1" style={{ overflow: 'hidden' }}>
          {/* Table header */}
          <div style={{ display: 'grid', gridTemplateColumns: '2.2fr 1fr 1.1fr 1fr 1fr 0.5fr 1fr 1fr', gap: 8, padding: '12px 20px', background: 'var(--surface2)', borderBottom: '1px solid var(--border)' }}>
          ...
          {loading ? (
            ...
            <div key={ath.id} style={{ display: 'grid', gridTemplateColumns: '2.2fr 1fr 1.1fr 1fr 1fr 0.5fr 1fr 1fr', gap: 8, alignItems: 'center', padding: '12px 20px', borderBottom: '1px solid var(--border)', transition: 'var(--transition)' }}
              onMouseEnter={e => e.currentTarget.style.background = 'var(--surface2)'}
              onMouseLeave={e => e.currentTarget.style.background = ''}>

              {/* Athlete with photo */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <AthleteAvatar athlete={ath} size={40} index={i} />
                <div>
                  <Link href={`/athletes/${ath.id}`} style={{ fontSize: 14, fontWeight: 700, color: 'var(--blue-dark)', display: 'block', marginBottom: 1 }}>
                    {ath.name}
                  </Link>
              ...
              <div><Badge status={ath.position || 'Forward'} type="sport" /></div>
              <div style={{ fontSize: 13, color: 'var(--text)' }}>{ath.club || '—'}</div>
              <div style={{ fontSize: 13, color: 'var(--text2)' }}>{ath.region || '—'}</div>
              <div style={{ fontSize: 12, color: 'var(--text2)' }}>{ath.coaches?.name?.replace('Coach ', '') || '—'}</div>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text2)' }}>{ath.age || '—'}</div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(44,62,80,0.6)', backdropFilter: 'blur(6px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--r-xl)', width: '100%', maxWidth: 580, maxHeight: '92vh', overflow: 'auto', boxShadow: 'var(--shadow-lg)', border: '1px solid var(--border)' }}>
            ...
            <div style={{ background: 'linear-gradient(90deg, #2E6FC4, #4A90E2)', padding: '20px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 4 }}>{editId ? 'Edit Record' : 'New Registration'}</div>
                <h2 style={{ fontSize: 20, fontWeight: 800, color: '#fff' }}>{editId ? 'Edit Athlete' : 'Register Athlete'}</h2>
              </div>
            ...
            <div style={{ padding: 28, display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {/* Photo upload */}
              <div>
                <label style={lbl}>Passport / Profile Photo</label>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'var(--surface2)', border: '3px dashed var(--border-md)', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                    {photoPreview
                  ...
                  <div>
                    <label htmlFor="photo-upload" style={{ display: 'inline-block', background: 'var(--blue-light)', color: 'var(--blue)', border: '1px solid rgba(74,144,226,0.3)', padding: '8px 18px', borderRadius: 'var(--r-sm)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                      {photoPreview ? 'Change Photo' : 'Upload Photo'}
                    </label>
                    <input id="photo-upload" type="file" accept="image/*" onChange={handlePhotoChange} style={{ display: 'none' }} />
                    {photoPreview && (
                      <button onClick={() => { setPhotoFile(null); setPhotoPreview(null) }} style={{ marginLeft: 8, background: 'var(--danger-light)', color: 'var(--danger)', border: 'none', padding: '7px 14px', borderRadius: 'var(--r-sm)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font)' }}>Remove</button>
                    )}
                    <p style={{ fontSize: 11, color: 'var(--text3)', marginTop: 5 }}>JPG or PNG. Passport style photo recommended.</p>
                  </div>
                </div>
              </div>
              ...
              <div>
  <label style={{ display:'block', fontSize:11, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', color:'var(--text3)', marginBottom:6 }}>
    Club / Team
  </label>
  <input
    value={form.club}
    onChange={e => set('club')(e.target.value)}
    style={{ width:'100%', padding:'10px 14px', background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:'var(--r-md)', fontSize:14, outline:'none', color:'var(--text)', fontFamily:'var(--font)' }}
    placeholder="e.g. Asante Kotoko SC"
    onFocus={e => e.target.style.borderColor = 'var(--blue)'}
    onBlur={e  => e.target.style.borderColor = 'var(--border)'}
  />
</div>
