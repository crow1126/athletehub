...
const PLANS = {
  starter: {
    label:'Starter', price:350, usd:23, color:'#381932', bg:'#EDD9C8', popular:false,
  ...
  academy: {
    label:'Academy', price:600, usd:40, color:'#065A82', bg:'#E6F0F8', popular:true,
  ...
  elite: {
    label:'Elite', price:1000, usd:67, color:'#1B7A3E', bg:'#E8F8EE', popular:false,
...
const STATUS_COLORS = { active:{bg:'#E8F8EE',color:'#1B7A3E'}, trial:{bg:'#EDD9C8',color:'#381932'}, expired:{bg:'#FDEDEC',color:'#C0392B'}, cancelled:{bg:'#FDEDEC',color:'#C0392B'} }
      ...
      <div style={{maxWidth:960,margin:'0 auto',padding:'32px 40px'}}>
        ...
        {upgradeReason==='upgrade_required'&&blockedModule&&(
          <div style={{background:'#FEF9E7',border:'1px solid rgba(183,119,13,0.3)',borderRadius:'var(--r-lg)',padding:'14px 20px',marginBottom:20,display:'flex',alignItems:'center',gap:12}}>
            ...
            <button onClick={()=>setTab('upgrade')} style={{padding:'8px 16px',background:'#B7770D',color:'#fff',border:'none',borderRadius:'var(--r-md)',fontWeight:700,fontSize:12,cursor:'pointer',fontFamily:'var(--font)',flexShrink:0}}>Upgrade →</button>
          </div>
        )}
        ...
        {tab==='overview'&&(
          <div style={{display:'flex',flexDirection:'column',gap:20}}>
            <div className="card" style={{padding:0,overflow:'hidden'}}>
              <div style={{background:currentPlan?.color||'var(--plum)',padding:'20px 24px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <div>
                  ...
                  <div style={{fontSize:28,fontWeight:800,color:'#FFF3E6'}}>{currentPlan?.label||sub?.plan||'Trial'}</div>
                  <div style={{fontSize:12,color:'rgba(255,243,230,0.6)',marginTop:4}}>{planLimits?.description}</div>
                </div>
                <div style={{textAlign:'right'}}>
                  <div style={{fontSize:34,fontWeight:900,color:'#FFF3E6'}}>{sub?.plan==='trial'?'FREE':`GHS ${currentPlan?.price?.toLocaleString()}`}</div>
                  {sub?.plan!=='trial'&&<div style={{fontSize:11,color:'rgba(255,243,230,0.6)'}}>per month · ≈ ${currentPlan?.usd} USD</div>}
                </div>
              </div>
              ...
              <div style={{borderTop:'1px solid var(--border)',padding:'16px 24px',display:'grid',gridTemplateColumns:currentPlan?.locked?.length>0?'1fr 1fr':'1fr',gap:16}}>
                ...
                {currentPlan?.locked?.length>0&&(
                  <div>
                    ...
                    <button onClick={()=>setTab('upgrade')} style={{marginTop:12,padding:'7px 16px',background:'var(--plum)',color:'var(--milk)',border:'none',borderRadius:'var(--r-md)',fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'var(--font)'}}>
                      Upgrade now →
                    </button>
                  </div>
                )}
              ...
              {isAdmin&&(
                <div style={{borderTop:'1px solid var(--border)',padding:'12px 24px',display:'flex',gap:10}}>
                  ...
                  {sub?.status==='active'&&sub?.plan!=='trial'&&(
                    <button onClick={handleCancel} style={{padding:'9px 20px',fontSize:13,fontWeight:600,background:'var(--danger-light)',color:'var(--danger)',border:'1px solid rgba(192,57,43,0.2)',borderRadius:'var(--r-md)',cursor:'pointer',fontFamily:'var(--font)'}}>Cancel</button>
                  )}
                </div>
              )}
            </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:22}}>
            ...
            <div className="card" style={{padding:'24px'}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:16,maxWidth:480}}>
                ...
                <button onClick={handlePaystack} disabled={paying||!isAdmin}
                  style={{background:paying?'var(--text3)':`linear-gradient(135deg,${PLANS[selPlan]?.color},${PLANS[selPlan]?.color}CC)`,color:'#fff',border:'none',padding:'14px 28px',borderRadius:'var(--r-md)',fontSize:15,fontWeight:700,cursor:paying||!isAdmin?'not-allowed':'pointer',fontFamily:'var(--font)',opacity:paying||!isAdmin?0.7:1,boxShadow:'0 4px 12px rgba(0,0,0,0.15)'}}>
