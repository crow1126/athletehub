        ...
        <div style={{ position:'relative', zIndex:2, width:'100%', maxWidth:420, animation:'fadeUp 0.5s cubic-bezier(0.22,1,0.36,1) forwards' }}>
          ...
          <div style={{ background:'#FFFFFF', border:'1px solid #E2E8F0', borderRadius:'0 0 22px 22px', borderTop:'none', boxShadow:'0 20px 48px rgba(15,23,42,0.06)', padding:'32px 28px' }}>
            <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:20 }}>
              <div>
                <h2 style={{ fontSize:22, fontWeight:800, color:'#0F172A', letterSpacing:'-0.02em', marginBottom:4 }}>Reset your password</h2>
                <p style={{ fontSize:13, color:'#64748B' }}>We'll send you a recovery link to update your account password.</p>
              </div>
              ...
              <div>
                <label style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.05em', color:'#64748B', marginBottom:6, display:'block' }}>Email Address</label>
                <input
                  type="email"
                  value={email}
              ...
              >
                {loading
                  ? <><span style={{ width:15, height:15, border:'2px solid rgba(255,255,255,0.35)', borderTopColor:'#FFFFFF', borderRadius:'50%', animation:'spin 0.6s linear infinite', display:'inline-block' }} /> Sending…</>
                  : 'Send Reset Link →'
                }
              </button>

              <p style={{ textAlign:'center', fontSize:12, color:'#64748B' }}>
                <Link href="/login" style={{ color:'#94A3B8', fontWeight:600, textDecoration:'none' }}>← Back to Sign In</Link>
              </p>
            </form>
          </div>
        </div>
