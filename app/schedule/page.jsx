    ...
    <Layout>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 40px' }}>
        <PageHeader
          ...
          title="Schedule"
          subtitle="Manage and track all training sessions"
          action={
            <button className="btn-blue" onClick={() => openAdd()}>+ New Session</button>
          }
        />
        ...
        <div className="fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 700, minWidth: 200, textAlign: 'center' }}>{MONTHS[month]} {year}</h2>
            <button onClick={() => setViewDate(d => new Date(d.getFullYear(), d.getMonth()+1, 1))} style={{ width: 36, height: 36, borderRadius: 'var(--r-md)', border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>›</button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 24, alignItems: 'start' }}>
            ...
            {view === 'month' ? (
              <div className="card fade-up" style={{ overflow: 'hidden' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', background: 'linear-gradient(90deg,#2E6FC4,#4A90E2)' }}>
                  {DAYS.map(d => (
                    <div key={d} style={{ padding: '12px 8px', textAlign: 'center', fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.9)', letterSpacing: '0.06em' }}>{d}</div>
                  ))}
                </div>
                {/* Calendar grid */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', background: 'var(--surface)' }}>
                  ...
                  {Array.from({ length: firstDay }).map((_, i) => (
                    <div key={`e${i}`} style={{ minHeight: 100, borderRight: '1px solid var(--border)', borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }} />
                  ))}
                  {/* Day cells */}
                  {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    return (
                      <div key={day} onClick={() => openAdd(dateStr)} style={{ minHeight: 100, borderRight: '1px solid var(--border)', borderBottom: '1px solid var(--border)', padding: '8px', cursor: 'pointer', transition: 'var(--transition)', background: isTod ? '#E8F4FF' : '' }}
                        ...
                        <div style={{ width: 26, height: 26, borderRadius: '50%', background: isTod ? 'var(--blue)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: isTod ? 800 : 500, color: isTod ? '#fff' : 'var(--text)', marginBottom: 4 }}>
                          {day}
                        </div>
                        {daySessions.slice(0,3).map(s => (
                          <div key={s.id} onClick={e => { e.stopPropagation(); openEdit(s) }}
                            style={{ fontSize: 10, fontWeight: 600, background: (COLORS[s.type]||'#4A90E2') + '20', color: COLORS[s.type]||'#4A90E2', padding: '2px 6px', borderRadius: 4, marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', borderLeft: `3px solid ${COLORS[s.type]||'#4A90E2'}`, cursor: 'pointer' }}>
                            {s.time} {s.title}
                          </div>
                        ))}
                        {daySessions.length > 3 && (
                          <div style={{ fontSize: 9, color: 'var(--text3)', fontWeight: 600 }}>+{daySessions.length-3} more</div>
                        )}
                      </div>
                    )
                  })}
              ...
              /* List view */
              <div className="card fade-up" style={{ overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '0.8fr 1.5fr 1fr 1fr 1fr 0.8fr 1fr', gap: 8, padding: '12px 20px', background: 'var(--surface2)', borderBottom: '1px solid var(--border)' }}>
                  {['Date','Session','Type','Venue','Coach','Duration','Actions'].map(h => (
                ...
                {sessions.length === 0 ? (
                  ...
                  return (
                    <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '0.8fr 1.5fr 1fr 1fr 1fr 0.8fr 1fr', gap: 8, alignItems: 'center', padding: '13px 20px', borderBottom: '1px solid var(--border)', transition: 'var(--transition)' }}
                      ...
                      <div>
                        <span style={{ fontSize: 11, fontWeight: 700, background: (COLORS[s.type]||'#4A90E2')+'20', color: COLORS[s.type]||'#4A90E2', padding: '3px 8px', borderRadius: 6 }}>{s.type}</span>
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--text2)' }}>📍 {s.venue}</div>
                      <div style={{ fontSize: 12, color: 'var(--text2)' }}>{coach?.name?.replace('Coach ','') || '—'}</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {upcoming.length === 0 ? (
                  ...
                  <div key={s.id} onClick={() => openEdit(s)} style={{ display: 'flex', gap: 12, padding: '10px 18px', borderBottom: '1px solid var(--border)', cursor: 'pointer', transition: 'var(--transition)' }}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text3)' }}>📍 {s.venue}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="card fade-up fade-up-1" style={{ padding: '16px 18px' }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', marginBottom: 12 }}>Session Types</h3>
            ...
            <div className="card fade-up fade-up-2" style={{ padding: '16px 18px' }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', marginBottom: 12 }}>This Month</h3>
              {(() => {
                const monthSessions = sessions.filter(s => { const d = new Date(s.date); return d.getMonth()===month && d.getFullYear()===year })
                const totalMins = monthSessions.reduce((acc,s) => acc + (parseInt(s.duration)||0), 0)
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {[['Sessions', monthSessions.length], ['Total Hours', Math.round(totalMins/60)+'h'], ['Venues', [...new Set(monthSessions.map(s=>s.venue))].length]].map(([l,v]) => (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(44,62,80,0.55)', backdropFilter: 'blur(6px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--r-xl)', width: '100%', maxWidth: 560, maxHeight: '92vh', overflow: 'auto', boxShadow: 'var(--shadow-lg)', border: '1px solid var(--border)' }}>
            <div style={{ padding: '20px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(90deg,#2E6FC4,#4A90E2)' }}>
              <div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 3 }}>Training Schedule</div>
            ...
            <div style={{ padding: 28, display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <input value={form.title} onChange={e => set('title')(e.target.value)} style={inp} placeholder="e.g. Morning Squad Training" />
              </div>
              <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
                ...
                <div>
                  ...
                  <input type="number" min="15" max="300" value={form.duration} onChange={e => set('duration')(e.target.value)} style={inp} />
                </div>
              </div>
              <div>
                <label style={lbl}>Venue</label>
                <select value={form.venue} onChange={e => set('venue')(e.target.value)} style={inp}>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={e => set('notes')(e.target.value)} rows={3} placeholder="Session details, objectives…" style={{ ...inp, resize: 'vertical' }} />
              </div>
