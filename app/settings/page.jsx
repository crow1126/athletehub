...
export default function SettingsPage() {
  ...
  const MsgBox = ({ m }) => !m.text ? null : (
    <div style={{ padding:'13px 16px',borderRadius:'var(--r-md)',fontSize:13,fontWeight:600,whiteSpace:'pre-line',lineHeight:1.7,marginTop:12,background:m.type==='error'?'var(--danger-light)':'var(--success-light)',color:m.type==='error'?'var(--danger)':'var(--success)',border:`1px solid ${m.type==='error'?'rgba(192,57,43,0.2)':'rgba(27,122,62,0.2)'}` }}>
      {m.text}
    </div>
  )
            ...
            {tab === 'profile' && (
              <div>
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:16,maxWidth:440 }}>
                  <div><label style={lbl}>Full Name</label><input value={profileForm.full_name} onChange={e=>setProfileForm(f=>({...f,full_name:e.target.value}))} style={inp} placeholder="Your full name" onFocus={onFocus} onBlur={onBlur}/></div>
            ...
            {tab === 'security' && (
              <div>
                ...
                {!isAdmin ? (
                  ...
                  <>
                    <p style={{ fontSize:14,color:'var(--text3)',marginBottom:24 }}>After changing your password you will be signed out and must log in again.</p>
            ...
            {tab === 'system' && isAdmin && (
              <div>
                ...
                <div className="system-grid" style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:24 }}>
                  {[['Platform','Apex Track v2.0'],['Database','Supabase PostgreSQL'],['Framework','Next.js 16'],['Auth','Supabase Auth (JWT)'],['Security','Club-scoped RLS'],['Region','West EU (London)']].map(([label,value])=>(
