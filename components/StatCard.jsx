  ...
  return (
    <div className="fade-up card" style={{ padding: '20px 22px', transition: 'var(--transition)', cursor: 'default', position: 'relative', overflow: 'hidden' }}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
        <div style={{ width: 44, height: 44, borderRadius: 12, background: accent ? `${accent}18` : 'var(--blue-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, border: `1px solid ${accent ? `${accent}28` : 'var(--border)'}` }}>
          {icon}
        </div>
        {trend && (
      ...
      {note && <div style={{ fontSize: 11, color: accent || 'var(--plum)', fontWeight: 600 }}>{note}</div>}
    </div>
  )
}