 ...
 * Format: CLUB NAME | [CATEGORY] Hi Name, Title — Message — ApexTrack
 */
export function buildNoticeSMS({ athleteName, clubName, title, message, category, authorName }) {
  const catTag = category === 'urgent'   ? '[URGENT]'   :
                 category === 'matchday' ? '[MATCHDAY]' :
                 category === 'training' ? '[TRAINING]' :
                 category === 'medical'  ? '[MEDICAL]'  : '[NOTICE]'

  const clubPrefix = clubName ? `${clubName.toUpperCase()} | ` : ''
  const greeting   = athleteName ? `Hi ${athleteName}, ` : ''

  // Keep message concise — trim to fit within 160 chars
  const body = `${message.length > 80 ? message.slice(0, 77) + '...' : message}`

  let msg = `${clubPrefix}${catTag} ${greeting}${title} — ${body} — ApexTrack`

  // If over 160, trim body further
  if (msg.length > 160) {
    const overhead = msg.length - body.length
    const allowed  = 160 - overhead - 3
    msg = `${clubPrefix}${catTag} ${greeting}${title} — ${message.slice(0, Math.max(20, allowed))}... — ApexTrack`
  }
