...
// Known registered club SMS packages (e.g. Young Apostles FC)
 ...
 * If the team has a custom key registered, it uses it with their custom header name.
 * If not provided or unregistered, it cleanly falls back to the default Apextrack Moolre SMS key.
 */
export async function resolveTeamSMSConfig(teamId, db = null) {
  ...
  if (REGISTERED_CLUB_SMS_KEYS[teamId]) {
    return REGISTERED_CLUB_SMS_KEYS[teamId]
  }
  ...
  if (client) {
    try {
      ...
      // Name match check (case-insensitive for Young Apostles)
      if (teamData?.name && teamData.name.toLowerCase().includes('young apostle')) {
        return {
          vasKey: 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ2YXNpZCI6MTMxMjAsImV4cCI6MTk1NjUyNzk5OX0.woIRGPdPX01MihjwzGViTKijuJLhOgxjOtLyOGCw2q4',
          senderId: 'Y APOSTLES'
        }
