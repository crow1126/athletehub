...
const REGISTERED_CLUB_SMS_KEYS = {
  ...
  '324cd849-5c62-4278-9594-97e606439402': {
    ...
    senderId: 'YAFC',
    senderIdApproved: true   // approved at app.moolre.com — 2026-09-04
  },
  // Kotoko SC
...
export async function resolveTeamSMSConfig(teamId, db = null) {
  ...
  if (client) {
    try {
      ...
      // While awaiting sender ID approval, don't use unapproved custom club keys from DB/subscriptions
      // to avoid Moolre rejecting the messages with "Sender ID not approved".
    } catch (err) {
