...
export function buildMatchdayCallupSMS({
  ...
  const base = `${greeting}you have been called up for match ${opponentStr}. Date: ${dateFormatted} | KO: ${formattedKickoff}${venueStr}${meetStr}. Report in full kit. — ${club}`
  
  if (base.length <= 160) return base
  
  // Compact fallback if over 160 chars
  const compact = `${greeting}you are called up ${opponentStr}. Date: ${dateFormatted} KO: ${formattedKickoff}${meetStr}. Report in full kit. — ${club}`
 ...
 */
export function buildNoticeSMS({ athleteName, clubName, title, message, category, authorName, matchDetails }) {
