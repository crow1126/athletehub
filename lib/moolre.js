...
// Until approved, SMS is sent via the club's own VAS key but with the default ApexTrack sender header.
const REGISTERED_CLUB_SMS_KEYS = {
  ...
  '324cd849-5c62-4278-9594-97e606439402': {
    ...
    senderId: 'Y APOSTLES',
    senderIdApproved: false  // ← set true once approved at app.moolre.com
  },
  // Kotoko SC
 ...
 * - Uses club's own VAS key (correct billing/routing)
 * - Uses club's custom senderId ONLY IF senderIdApproved = true (Moolre approval required)
 * - Falls back to ApexTrack sender name if custom ID is not yet approved
 */
export async function resolveTeamSMSConfig(teamId, db = null) {
  ...
  if (REGISTERED_CLUB_SMS_KEYS[teamId]) {
    ...
    return {
      vasKey: cfg.vasKey,
      // Only use custom sender if Moolre has approved it — else use default to avoid "Sender ID not approved" error
      senderId: cfg.senderIdApproved ? cfg.senderId : MOOLRE_SENDER
    }
  }
  ...
  if (client) {
    try {
      ...
      const { data: teamData } = await client
        .from('teams')
        .select('id, name, short_name, moolre_vas_key, sms_sender_id')
        .eq('id', teamId)
        .maybeSingle()

      if (teamData?.moolre_vas_key) {
        return {
          vasKey: teamData.moolre_vas_key.trim(),
          senderId: (teamData.sms_sender_id || teamData.short_name || teamData.name?.slice(0, 11) || MOOLRE_SENDER).trim()
        }
      }
      ...
      if (teamNameLower.includes('young apostle')) {
        return REGISTERED_CLUB_SMS_KEYS['324cd849-5c62-4278-9594-97e606439402']
      }
      if (teamNameLower.includes('kotoko')) {
        return REGISTERED_CLUB_SMS_KEYS['99801a63-c7ba-474d-a664-86de133ff054']
      }

      // 3. Check subscriptions table notes (contains persisted JSON metadata)
      ...
      if (subData?.notes) {
        try {
          const parsed = JSON.parse(subData.notes)
          if (parsed?.moolre_vas_key) {
            return {
              vasKey: parsed.moolre_vas_key.trim(),
              senderId: (parsed.sms_sender_id || teamData?.short_name || MOOLRE_SENDER).trim()
            }
          }
        } catch {
          // not JSON
  ...
  // 4. Clean fallback to default Apextrack SMS credentials
  return {
...
export function buildMatchdayCallupSMS({
  ...
  // Compact fallback if over 160 chars
  const compact = `${greeting}${actionText} ${opponentStr}. Date: ${dateFormatted} KO: ${formattedKickoff}${meetStr}. — ${club}`
