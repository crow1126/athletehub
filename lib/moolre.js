...
// Sandbox: X-API-KEY and X-API-PUBKEY are NOT required — only X-API-USER needed
const IS_SANDBOX = MOOLRE_BASE_URL.includes('sandbox.moolre.com')

// Headers for Payments (X-API-PUBKEY skipped in sandbox)
function paymentHeaders() {
  const headers = { 'Content-Type': 'application/json' }
  if (MOOLRE_USER)                    headers['X-API-USER']   = MOOLRE_USER
  if (MOOLRE_PUBKEY && !IS_SANDBOX)   headers['X-API-PUBKEY'] = MOOLRE_PUBKEY
  return headers
}

// Headers for Transfers/Disbursements (X-API-KEY skipped in sandbox)
function transferHeaders() {
  const headers = { 'Content-Type': 'application/json' }
  if (MOOLRE_USER)                    headers['X-API-USER'] = MOOLRE_USER
  if (MOOLRE_SECRET && !IS_SANDBOX)   headers['X-API-KEY']  = MOOLRE_SECRET
  return headers
}
...
export async function initiateTransfer({ amount, recipient, provider, reference }) {
  try {
    ...
    const body = {
      ...
      receiver:      normalizeGhPhone(recipient),
      externalref:   reference,
      accountnumber: MOOLRE_ACCOUNT,
    }
