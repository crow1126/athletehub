...
const MOOLRE_USER     = process.env.MOOLRE_API_USER?.trim()
const MOOLRE_PUBKEY   = process.env.MOOLRE_PUBLIC_KEY?.trim()
const MOOLRE_SECRET   = process.env.MOOLRE_SECRET_KEY?.trim()
const MOOLRE_VASKEY   = process.env.MOOLRE_VAS_KEY?.trim()
const MOOLRE_ACCOUNT  = process.env.MOOLRE_ACCOUNT_NUMBER?.trim()
...
/**
 * Resolve Ghanaian Mobile Network Operator prefix code
 */
export function resolveGhProvider(rawPhone) {
  const phone = normalizeGhPhone(rawPhone)
  if (!phone) return 'MTN'
  // Starts with 233, so extract prefix at digits 3 and 4 (0-indexed)
  ...
  if (telecelPrefixes.includes(prefix)) return 'VODAFONE'
  if (atPrefixes.includes(prefix)) return 'AIRTELTIGO'
...
export async function initiateTransfer({ amount, recipient, provider, reference }) {
  try {
    const resolvedProvider = provider || resolveGhProvider(recipient)
    const body = {
      type: 2, // 2 = Transfer
      amount: amount.toString(),
      recipient: normalizeGhPhone(recipient),
    ...
    const res = await fetch(`${MOOLRE_BASE_URL}/open/transfer`, {
      method:  'POST',
    ...
    const data = await res.json()
    if (!res.ok || data.status !== 1) {
      return { ok: false, error: data?.message || 'Transfer request failed' }
    }
    return { ok: true, data: data.data }
  } catch (err) {
    return { ok: false, error: err.message }
