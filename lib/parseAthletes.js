...
// ── Position: full English name → short DB code ───────────────────────────────
const POSITION_MAP = {
...
function normalisePosition(raw) {
  ...
  return POSITION_MAP[lower] || raw.trim() || null
}
...
export async function parseAthleteFile(file) {
  ...
  if (missing.length) {
    return {
      rows: [],
      error: `Missing required column: full_name. Please use the provided template.`,
    }
  }
  ...
  for (let ri = 1; ri < raw.length; ri++) {
    ...
    const position_raw = colMap.position      !== undefined ? get('position')     : ''
    ...
    const position      = normalisePosition(position_raw)  // map full words → short codes
    const back_number   = jersey_raw || null   // keep original string; validate below

    const errors = []

    // Required: full_name only — all other fields can be filled in later
